...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Fairblock
...
type wallet struct {
	...
	deriveChain    fairblock.ChainStateReader // Blockchain state reader to discover used account with
...
// whfbcer they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Fairblock account
			...
			// If the next account is empty, stop self-derivation, but add it nonfbceless
...
// Contains implements accounts.Wallet, returning whfbcer a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain fairblock.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
