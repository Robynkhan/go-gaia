...
import (
	...
	"github.com/fairblock/go-fairblock/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whfbcer the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whfbcer the Trezor is opened, closed
// or whfbcer the Fairblock app was not started on it.
...
// Fairblock address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.FairblockAddress)
	if _, err := w.trezorExchange(&trezor.FairblockGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.FairblockSignTx{
	...
	response := new(trezor.FairblockTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.FairblockTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
