...
type SimulatedBackend struct {
	database   fbcdb.Database   // In memory database to store our testing data
	blockchain *core.BlockChain // Fairblock blockchain to handle the consensus
...
func NewSimulatedBackend(alloc core.GenesisAlloc) *SimulatedBackend {
	database, _ := fbcdb.NewMemDatabase()
	genesis := core.Genesis{Config: params.AllFbcashProtocolChanges, Alloc: alloc}
	genesis.MustCommit(database)
	blockchain, _ := core.NewBlockChain(database, genesis.Config, fbcash.NewFaker(), vm.Config{})
...
func (b *SimulatedBackend) CallContract(ctx context.Context, call fairblock.CallMsg, blockNumber *big.Int) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) PendingCallContract(ctx context.Context, call fairblock.CallMsg) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call fairblock.CallMsg) (*big.Int, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContract(ctx context.Context, call fairblock.CallMsg, block *types.Block, statedb *state.StateDB) ([]byte, *big.Int, bool, error) {
