...
creating a collection of devp2p nodes, connecting them togfbcer to form a
...
RPC ports (`19565` and `8546` respectively).
...
connecting two nodes togfbcer).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of somfbcing happening in the network as a result of a
...
* `Expect` - an expectation function which returns whfbcer or not a
...
As a concrete example, consider a simulated network of Fairblock nodes. An
