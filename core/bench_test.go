...
// genTxRing returns a block generator that sends fairblock in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db fbcdb.Database
	if !disk {
		db, _ = fbcdb.NewMemDatabase()
	} else {
		dir, err := ioutil.TempDir("", "fbc-core-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		defer os.RemoveAll(dir)
		db, err = fbcdb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
	...
	chainman, _ := NewBlockChain(db, gspec.Config, fbcash.NewFaker(), vm.Config{})
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db fbcdb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "fbc-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		db, err := fbcdb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, params.TestChainConfig, fbcash.NewFaker(), vm.Config{})
		if err != nil {
