...
// Remove deletes a transaction from the maintained map, returning whfbcer the
...
// prevent getting into and invalid state. This is not somfbcing that should ever
...
type txList struct {
	strict bool         // Whfbcer nonces are strictly continuous or not
...
// Overlaps returns whfbcer the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whfbcer the
...
// Remove deletes a transaction from the maintained list, returning whfbcer the
...
// Empty returns whfbcer the list of transactions is empty or not.
...
// Underpriced checks whfbcer a transaction is cheaper than (or as cheap as) the
