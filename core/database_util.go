...
var (
	...
	configPrefix   = []byte("fairblock-config-") // config prefix for the db
...
func WriteCanonicalHash(db fbcdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db fbcdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db fbcdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db fbcdb.Putter, hash common.Hash) error {
...
func WriteHeader(db fbcdb.Putter, header *types.Header) error {
...
func WriteBody(db fbcdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db fbcdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db fbcdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db fbcdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db fbcdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db fbcdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db fbcdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db fbcdb.Database) fbcdb.Database {
	return fbcdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db fbcdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db fbcdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db fbcdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
