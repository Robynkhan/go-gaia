...
const (
	...
	fbcVersion = 63 // equivalent fbc version for the downloader
...
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, protocolVersions []uint, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb fbcdb.Database, odr *LesOdr, txrelay *LesTxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Fairblock protocol")
	...
	log.Info("Light Fairblock protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Fairblock peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Fairblock message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Fairblock message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	case GetHeaderProofsMsg:
		...
		trieDb := fbcdb.NewTable(pm.chainDb, light.ChtTablePrefix)
		for _, req := range req.Reqs {
			if bytes >= softResponseLimit {
				break
			}
	...
	case GetHelperTrieProofsMsg:
		...
		for _, req := range req.Reqs {
			...
			if tr == nil || req.HelperTrieType != lastType || req.TrieIdx != lastIdx {
				...
				if root != (common.Hash{}) {
					if t, err := trie.New(root, fbcdb.NewTable(pm.chainDb, prefix)); err == nil {
						tr = t
					}
				}
...
func (self *ProtocolManager) NodeInfo() *fbc.FbcNodeInfo {
	return &fbc.FbcNodeInfo{
...
func (d *downloaderPeerNotify) registerPeer(p *peer) {
	...
	pm.downloader.RegisterLightPeer(p.id, fbcVersion, pc)
}
