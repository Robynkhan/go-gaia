...
import (
	...
	"github.com/fairblock/go-fairblock/les/flowcontrol"
...
func NewLesServer(fbc *fbc.Fairblock, config *fbc.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(fbc.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, fbc.EventMux(), fbc.Engine(), newPeerSet(), fbc.BlockChain(), fbc.TxPool(), fbc.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range ServerProtocolVersions {
		lesTopics[i] = lesTopic(fbc.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(fbc.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(fbc.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(fbc.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(fbc.ChainDb())
	return srv, nil
}
...
func newCostStats(db fbcdb.Database) *requestCostStats {
