...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gfbc.LightScryptN, Gfbc.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.fairblock", new NodeConfig());
			node.start();
			...
			FairblockClient ec = node.getFairblockClient();
	...
	// signatures too. Regression test for go-fairblock issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogfbcer
	...
	workspace, err := ioutil.TempDir("", "gfbc-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/fairblock/gfbctest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gfbc and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.fairblock", "github.com/fairblock/go-fairblock/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gfbc.aar"), "gfbc.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "fairblock", "gfbctest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.fairblock.gfbctest"
