...
// Package gfbc contains the simplified mobile APIs to go-fairblock.
//
// The scope of this package is *not* to allow writing a custom Fairblock client
// with pieces plucked from go-fairblock, rather to allow writing native dapps on
...
// from the go-fairblock repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-fairblock version.
