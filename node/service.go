...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (fbcdb.Database, error) {
	if ctx.config.DataDir == "" {
		return fbcdb.NewMemDatabase()
	}
	db, err := fbcdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
