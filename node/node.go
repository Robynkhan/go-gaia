...
func (n *Node) OpenDatabase(name string, cache, handles int) (fbcdb.Database, error) {
	if n.config.DataDir == "" {
		return fbcdb.NewMemDatabase()
	}
	return fbcdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
