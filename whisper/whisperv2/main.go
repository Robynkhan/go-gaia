...
import (
	...
	"github.com/fairblock/go-fairblock/logger"
	"github.com/fairblock/go-fairblock/p2p"
	"github.com/fairblock/go-fairblock/p2p/nat"
	"github.com/fairblock/go-fairblock/whisper"
)
...
func main() {
	...
	// Create an Fairblock peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting Fairblock peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start Fairblock peer: %v.\n", err)
		os.Exit(1)
	}

	// Send a message to self to check that somfbcing works
