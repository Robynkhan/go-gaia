...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"fbcstats", "explorer", "wallet", "faucet"} {
	...
	// If we have fbcstats running, ask whfbcer to make the secret public or not
	var fbcstats bool
	if w.conf.fbcstats != "" {
		fmt.Println()
		fmt.Println("Include fbcstats secret on dashboard (y/n)? (default = yes)")
		fbcstats = w.readDefaultString("y") == "y"
	}
	// Try to deploy the dashboard container on the host
	if out, err := deployDashboard(client, w.network, infos.port, infos.host, listing, &w.conf, fbcstats); err != nil {
