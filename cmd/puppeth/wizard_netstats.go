...
func (w *wizard) networkStats(tips bool) {
	...
	for server, pubkey := range w.conf.Servers {
		...
		logger.Debug("Checking for fbcstats availability")
		if infos, err := checkFbcstats(client, w.network); err != nil {
			if err != ErrServiceUnknown {
				services["fbcstats"] = err.Error()
			}
		} else {
			services["fbcstats"] = infos.String()
			protips.fbcstats = infos.config
		}
	...
	if protips.fbcstats != "" {
		w.conf.fbcstats = protips.fbcstats
	}
...
func (p *protips) print(network string) {
	...
	fullinit, lightinit := "", ""
	if p.genesis != "" {
		fullinit = fmt.Sprintf("gfbc --datadir=$HOME/.%s init %s.json && ", network, network)
		lightinit = fmt.Sprintf("gfbc --datadir=$HOME/.%s --light init %s.json && ", network, network)
	}
	// If an fbcstats server is available, add the fbcstats flag
	statsflag := ""
	if p.fbcstats != "" {
		if strings.Contains(p.fbcstats, " ") {
			statsflag = fmt.Sprintf(` --fbcstats="yournode:%s"`, p.fbcstats)
		} else {
			statsflag = fmt.Sprintf(` --fbcstats=yournode:%s`, p.fbcstats)
		}
	}
	...
	tips = append(tips, fmt.Sprintf("%sgfbc --networkid=%d --datadir=$HOME/.%s --cache=1024%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a full node with recent data only")
	tips = append(tips, fmt.Sprintf("%sgfbc --networkid=%d --datadir=$HOME/.%s --cache=512 --fast%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a light node with on demand retrievals")
	tips = append(tips, fmt.Sprintf("%sgfbc --networkid=%d --datadir=$HOME/.%s --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))

	tasks = append(tasks, "Run an embedded node with constrained memory")
	tips = append(tips, fmt.Sprintf("%sgfbc --networkid=%d --datadir=$HOME/.%s --cache=32 --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))
