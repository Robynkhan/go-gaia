...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/fairblock                                                            && \
	(cd $GOPATH/src/github.com/fairblock && git clone --depth=1 https://github.com/fairblock/go-fairblock) && \
  go build -v github.com/fairblock/go-fairblock/cmd/faucet                                              && \
...
CMD [ \
	"/faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--fbcstats", "{{.Fbcstats}}", "--fbcport", "{{.FbcPort}}", \
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Fbcstats":      config.node.fbcstats,
		"FbcPort":       config.node.portFull,
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"FbcPort":       config.node.portFull,
		"FbcName":       config.node.fbcstats[:strings.Index(config.node.fbcstats, ":")],
		"GitHubUser":    config.githubUser,
		"GitHubToken":   config.githubToken,
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
...
func (info *faucetInfos) String() string {
	return fmt.Sprintf("host=%s, api=%d, fbc=%d, amount=%d, minutes=%d, tiers=%d, github=%s, captcha=%v, fbcstats=%s", info.host, info.port, info.node.portFull, info.amount, info.minutes, info.tiers, info.githubUser, info.captchaToken != "", info.node.fbcstats)
}

// checkFaucet does a health-check against an faucet server to verify whfbcer
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			portFull: infos.portmap[infos.envvars["FBC_PORT"]+"/tcp"],
			fbcstats: infos.envvars["FBC_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
