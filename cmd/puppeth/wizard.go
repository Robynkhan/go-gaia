...
// config contains all the configurations needed by puppfbc that should be saved
// between sessions.
type config struct {
	...
	fbcstats  string        // Fbcstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppfbc configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Fairblock services known to be running on servers
...
// converts it to an Fairblock address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by fbcstats, which compares IPs textually, not by value.
