...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.fbcstats == "" {
		log.Error("No fbcstats server configured")
		return
	}
	...
	// Retrieve any active fbcstats configurations from the server
	infos, err := checkNode(client, w.network, boot)
	if err != nil {
		if boot {
			infos = &nodeInfos{portFull: 19565, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{portFull: 19565, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	...
	if infos.fbcstats == "" {
		...
		infos.fbcstats = w.readString() + ":" + w.conf.fbcstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.fbcstats)
		infos.fbcstats = w.readDefaultString(infos.fbcstats) + ":" + w.conf.fbcstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.genesis.Config.Fbcash != nil {
			// Fbcash based miners only need an fbcerbase to mine against
			fmt.Println()
			if infos.fbcerbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.fbcerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.fbcerbase)
				infos.fbcerbase = w.readDefaultAddress(common.HexToAddress(infos.fbcerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy Fairblock node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
