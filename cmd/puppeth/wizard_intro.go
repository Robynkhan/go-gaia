...
// makeWizard creates and returns a new puppfbc wizard.
...
// setting up a new or managing an existing Fairblock private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppfbc, your Fairblock private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Fairblock network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and fbcstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppfbc uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Fairblock network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppfbc", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somfbcing I can do")
		}
	}
}
