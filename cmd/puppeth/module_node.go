...
// nodeDockerfile is the Dockerfile required to run an Fairblock node.
var nodeDockerfile = `
FROM fairblock/client-go:latest
...
{{end}}
RUN \
  echo 'gfbc init /genesis.json' > gfbc.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.fairblock/keystore/ && cp /signer.json /root/.fairblock/keystore/' >> gfbc.sh && \{{end}}
	echo $'gfbc --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --fbcstats \'{{.Fbcstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Fairblockbase}}--fbcerbase {{.Fairblockbase}} --mine{{end}}{{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> gfbc.sh

ENTRYPOINT ["/bin/sh", "gfbc.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Fairblock node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
func deployNode(client *sshClient, network string, bootv4, bootv5 []string, config *nodeInfos) ([]byte, error) {
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Fbcstats":  config.fbcstats,
		"Fairblockbase": config.fbcerbase,
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Fbcstats":   config.fbcstats[:strings.Index(config.fbcstats, ":")],
		"Fairblockbase":  config.fbcerbase,
...
func (info *nodeInfos) String() string {
	...
	return fmt.Sprintf("port=%d%s, datadir=%s, peers=%d, lights=%d, fbcstats=%s, gastarget=%0.3f MGas, gasprice=%0.3f GWei",
		info.portFull, discv5, info.datadir, info.peersTotal, info.peersLight, info.fbcstats, info.gasTarget, info.gasPrice)
}

// checkNode does a health-check against an boot or seal node server to verify
// whfbcer it's running, and if yes, whfbcer it's responsive.
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gfbc --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.fairblock"],
		portFull:   infos.portmap[infos.envvars["FULL_PORT"]],
		portLight:  infos.portmap[infos.envvars["LIGHT_PORT"]],
		peersTotal: totalPeers,
		peersLight: lightPeers,
		fbcstats:   infos.envvars["STATS_NAME"],
		fbcerbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasPrice:   gasPrice,
