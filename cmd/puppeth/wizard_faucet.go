...
func (w *wizard) deployFaucet() {
	...
	if err != nil {
		infos = &faucetInfos{
			node:    &nodeInfos{portFull: 19565, peersTotal: 25},
			port:    80,
			host:    client.server,
			amount:  1,
			minutes: 1440,
	...
	fmt.Println()
	fmt.Printf("How many Fairblocks to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.fbcstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.fbcstats = w.readString() + ":" + w.conf.fbcstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.fbcstats)
		infos.node.fbcstats = w.readDefaultString(infos.node.fbcstats) + ":" + w.conf.fbcstats
	}
