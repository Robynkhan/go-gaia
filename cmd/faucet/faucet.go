...
// faucet is a Fairblock faucet backed by a light client.
package main
...
var (
	...
	fbcPortFlag = flag.Int("fbcport", 19565, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Fairblock protocol")
	statsFlag   = flag.String("fbcstats", "", "Fbcstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Fairblocks to pay out per user request")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for Fairblock and the faucet")
)

var (
	fairblock = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Fairblocks", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *fbcPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Fairblock address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Fairblock light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Fairblock protocol stack
	client *fbcclient.Client   // Client connection to the Fairblock chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Fairblock light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := fbc.DefaultConfig
	...
	// Assemble the fbcstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightFairblock
			ctx.Service(&serv)
			return fbcstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := fbcclient.NewClient(api)
...
// close terminates the Fairblock connection and tears down the faucet.
...
// apiHandler handles requests for Fairblock grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, fairblock),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Fairblock address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somfbcing funky happened, please open an issue at https://github.com/fairblock/go-fairblock/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), fairblock)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance = new(big.Int).Div(balance, fairblock)
...
// the username, avatar URL and Fairblock address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Fairblock addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Fairblock address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Fairblock address and profile URL.
...
// returning the username, avatar URL and Fairblock address to fund on success.
