...
#
# The cluster contains a bootnode, a gfbc node and multiple swarm nodes, with
...
# static gfbc configuration
GFBC_IP="192.168.33.3"
GFBC_RPC_PORT="9565"
GFBC_RPC_URL="http://${GFBC_IP}:${GFBC_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gfbc and
# swarm nodes
start_bootnode() {
...
# start_gfbc_node starts a gfbc node with --datadir pointing at <base-dir>/gfbc
# and a single, unlocked account with password "gfbc"
start_gfbc_node() {
  local dir="${base_dir}/gfbc"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gfbc account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GFBC_IP}"
    --rpcport   "${GFBC_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gfbc" "${GFBC_IP}" "$(which gfbc)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vfbc pair
  local vfbc0="vfbc${name}0"
  local vfbc1="vfbc${name}1"
  ip link add name "${vfbc0}" type vfbc peer name "${vfbc1}"

  # add one end to the bridge
  ip link set dev "${vfbc0}" master "${BRIDGE_NAME}"
  ip link set dev "${vfbc0}" up

  # add the other end to the namespace, rename it fbc0 and give it the ip
  ip link set dev "${vfbc1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vfbc1}" name "fbc0"
  ip netns exec "${name}" ip link set dev "fbc0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "fbc0"
}
...
create_account() {
  ...
  gfbc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
