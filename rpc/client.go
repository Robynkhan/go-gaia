...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout fbc_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for FbcSubscribe requests
}
...
// FbcSubscribe registers a subscripion under the "fbc" namespace.
func (c *Client) FbcSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "fbc", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. FbcSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through FbcSubscribe.
