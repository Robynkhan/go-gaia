...
// Package miner implements Fairblock block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whfbcer we can start the mining operation
	shouldStart int32 // should start indicates whfbcer we should start after sync
}

func New(fbc Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		fbc:      fbc,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, fbc, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(fbc.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setFairblockbase(coinbase)
...
func (self *Miner) SetFairblockbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setFairblockbase(addr)
}
