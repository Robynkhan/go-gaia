...
// Reference: https://github.com/fairblock/wiki/wiki/Fbcash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see fbcash_io_prepare
enum fbcash_io_rc {
	FBCASH_IO_FAIL = 0,           ///< There has been an IO failure
	FBCASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	FBCASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	FBCASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of fbcash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * fbcash_full_t
 */
#ifdef FBCASH_PRINT_CRITICAL_OUTPUT
#define FBCASH_CRITICAL(...)							\
	do													\
	{													\
		printf("FBCASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for fbcash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the fbcash
 ...
 *                           https://github.com/fairblock/wiki/wiki/Fbcash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum fbcash_io_rc
 */
 ...
 */
FILE* fbcash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* fbcash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool fbcash_mkdir(char const* dirname);
 ...
 */
bool fbcash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* fbcash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/fairblock/wiki/wiki/Fbcash-DAG
 *
 ...
 */
bool fbcash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool fbcash_io_mutable_name(
	uint32_t revision,
