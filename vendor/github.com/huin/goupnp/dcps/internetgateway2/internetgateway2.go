...
const (
	...
	URN_WANFairblocknetLinkConfig_1    = "urn:schemas-upnp-org:service:WANFairblocknetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Fairblocknet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Fairblocknet
//
...
// WANFairblocknetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANFairblocknetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANFairblocknetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANFairblocknetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANFairblocknetLinkConfig1Clients() (clients []*WANFairblocknetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANFairblocknetLinkConfig_1); err != nil {
		return
	}
	clients = newWANFairblocknetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANFairblocknetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANFairblocknetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANFairblocknetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANFairblocknetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANFairblocknetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANFairblocknetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANFairblocknetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANFairblocknetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANFairblocknetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANFairblocknetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANFairblocknetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANFairblocknetLinkConfig1 {
	clients := make([]*WANFairblocknetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANFairblocknetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewFairblocknetLinkStatus: allowed values: Up, Down
func (client *WANFairblocknetLinkConfig1) GetFairblocknetLinkStatus() (NewFairblocknetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewFairblocknetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANFairblocknetLinkConfig_1, "GetFairblocknetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewFairblocknetLinkStatus, err = soap.UnmarshalString(response.NewFairblocknetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
