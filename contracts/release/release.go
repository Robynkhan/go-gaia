...
type Config struct {
	Oracle common.Address // Fairblock address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the Fairblock service dependency to access the blockchain
	var apiBackend fbcapi.Backend
	var fairblock *fbc.Fairblock
	if err := ctx.Service(&fairblock); err == nil {
		apiBackend = fairblock.ApiBackend
	} else {
		var fairblock *les.LightFairblock
		if err := ctx.Service(&fairblock); err == nil {
			apiBackend = fairblock.ApiBackend
		} else {
			return nil, err
	...
	contract, err := NewReleaseOracle(config.Oracle, fbc.NewContractBackend(apiBackend))
	if err != nil {
		return nil, err
	}
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/fairblock/go-fairblock/releases for new releases")
