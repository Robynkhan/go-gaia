...
func (b *testBackend) ChainDb() fbcdb.Database {
	return b.db
}
...
// TestPendingTxFilter tests whfbcer pending tx filters retrieve all pending transactions that are posted to the event mux.
...
// TestLogFilterCreation test whfbcer a given filter criteria makes sense.
...
// TestInvalidLogFilterCreation tests whfbcer invalid filter log criteria results in an error
...
// TestLogFilter tests whfbcer log filters match the correct logs that are posted to the event feed.
