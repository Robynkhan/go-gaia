...
func testGetNodeData(t *testing.T, protocol int) {
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*fbcdb.MemDatabase).Keys() {
	...
	statedb, _ := fbcdb.NewMemDatabase()
...
// Tests that post fbc protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	peer, _ := newTestPeer("peer", fbc63, pm, true)
	defer peer.close()
