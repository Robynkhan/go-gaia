...
type peerConnection struct {
	...
	version int        // Fbc protocol version number to switch strategies
...
func (p *peerConnection) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [fbc/62+] requested on fbc/%d", p.version))
	}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [fbc/62+] requested on fbc/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [fbc/63+] requested on fbc/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [fbc/63+] requested on fbc/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
// Lacks retrieves whfbcer the hash of a blockchain item is on the peers lacking
// list (i.e. whfbcer we know that the peer does not have it).
