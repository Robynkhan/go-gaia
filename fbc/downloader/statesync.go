...
// stateReq represents a batch of state fetch requests groupped togfbcer into
...
type stateReq struct {
	...
	dropped  bool                       // Flag whfbcer the peer dropped off early
}
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting somfbcing else, ignore the stale timeout.
...
func (s *stateSync) loop() error {
	...
	for s.sched.Pending() > 0 {
		...
		s.assignTasks()
		// Tasks assigned, wait for somfbcing to happen
		select {
		case <-newPeer:
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < fbcdb.IdealBatchSize {
		return nil
	}
	start := time.Now()
...
func (s *stateSync) process(req *stateReq) (bool, error) {
	...
	for hash, task := range req.tasks {
		// If the node did deliver somfbcing, missing items may be due to a protocol
...
// peer into the state trie, returning whfbcer anything useful was written or any
// error occurred.
