...
import (
	...
	fairblock "github.com/fairblock/go-fairblock"
...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [fbc/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [fbc/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [fbc/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [fbc/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [fbc/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [fbc/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [fbc/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [fbc/63] Channel receiving inbound node state data
...
func New(mode SyncMode, stateDb fbcdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
// these are zero.
func (d *Downloader) Progress() fairblock.SyncProgress {
...
// Synchronising returns whfbcer the downloader is currently retrieving blocks.
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "fbc", p.version, "head", hash, "td", td, "mode", d.mode)
...
func (d *Downloader) fetchHeight(p *peerConnection) (*types.Header, error) {
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave somfbcing valid
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered somfbcing completely else than requested (usually
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have somfbcing)
				// R: Nothing to give
				if d.mode != LightSync {
				...
				// peer gave us somfbcing useful, we're already happy/progressed (above check).
			...
			for len(headers) > 0 {
				// Terminate if somfbcing failed in between processing chunks
				select {
				case <-d.cancelCh:
					return errCancelHeaderProcessing
				default:
