...
import (
	...
	"github.com/fairblock/go-fairblock/common/bitutil"
...
func GetChtRoot(db fbcdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func GetChtV2Root(db fbcdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	return GetChtRoot(db, (sectionIdx+1)*(ChtFrequency/ChtV1Frequency)-1, sectionHead)
}

// StoreChtRoot writes the CHT root assoctiated to the given section into the database
// Note that sectionIdx is specified according to LES/1 CHT section size
func StoreChtRoot(db fbcdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewChtIndexer(db fbcdb.Database, clientMode bool) *core.ChainIndexer {
	cdb := fbcdb.NewTable(db, ChtTablePrefix)
	idb := fbcdb.NewTable(db, "chtIndex-")
...
const (
	...
	fbcBloomBitsSection       = 4096
	fbcBloomBitsConfirmations = 256
)
...
func GetBloomTrieRoot(db fbcdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db fbcdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
// NewBloomTrieIndexer creates a BloomTrie chain indexer
func NewBloomTrieIndexer(db fbcdb.Database, clientMode bool) *core.ChainIndexer {
	cdb := fbcdb.NewTable(db, BloomTrieTablePrefix)
	idb := fbcdb.NewTable(db, "bltIndex-")
	...
	if clientMode {
		...
		backend.parentSectionSize = fbcBloomBitsSection
		confirmReq = HelperTrieProcessConfirmations
	}
	backend.bloomTrieRatio = BloomTrieFrequency / backend.parentSectionSize
	backend.sectionHeads = make([]common.Hash, backend.bloomTrieRatio)
	return core.NewChainIndexer(db, idb, backend, BloomTrieFrequency, confirmReq-fbcBloomBitsConfirmations, time.Millisecond*100, "bloomtrie")
}
