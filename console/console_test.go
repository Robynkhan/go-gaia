...
func newTester(t *testing.T, confOverride func(*fbc.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Fairblock service within
	...
	fbcConf := &fbc.Config{
		...
		Fairblockbase: common.HexToAddress(testAddress),
		PowTest:   true,
	}
	if confOverride != nil {
		confOverride(fbcConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return fbc.New(ctx, fbcConf) }); err != nil {
		t.Fatalf("failed to register Fairblock protocol: %v", err)
	}
