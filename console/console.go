...
import (
	...
	"github.com/fairblock/go-fairblock/internal/jsre"
	"github.com/fairblock/go-fairblock/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Fairblock requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jfbc", struct{}{})

	jfbcObj, _ := c.jsre.Get("jfbc")
	jfbcObj.Object().Set("send", bridge.Send)
	jfbcObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jfbc);"); err != nil {
	...
	flatten := "var fbc = web3.fbc; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = fbc.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jfbc.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jfbc.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jfbc.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jfbc.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jfbc.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines fbc.getBalance(fbc.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gfbc instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gfbc metadata
	fmt.Fprintf(c.printer, "Welcome to the Gfbc JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + fbc.coinbase);
		console.log("at block: " + fbc.blockNumber + " (" + new Date(1000 * fbc.getBlock(fbc.blockNumber).timestamp) + ")");
